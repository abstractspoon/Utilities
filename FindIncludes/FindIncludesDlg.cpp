...
BOOL CFindIncludesDlg::OnInitDialog()
{
	...
	m_groupResults.Initialize(::GetDlgItem(*this, IDC_RESULTSLABEL));
...
void CFindIncludesDlg::DoSearch() 
{
	...
	CString sResults;
	sResults.Format(_T("Results (%d found)"), nNumRes);
	GetDlgItem(IDC_RESULTSLABEL)->SetWindowText(sResults);
}

int CFindIncludesDlg::DoSearch(INCLUDEFILEITEM& fi, HTREEITEM htiParent, CMap<CString, LPCTSTR, BOOL, BOOL&>& mapVisited) 
{
		...
		for (int nIncl = 0; nIncl < nNumInclude; nIncl++)
		{
			...
			if (m_bRecursive)
			{
				...
				if (m_mapFile2IncludeItem.Lookup(sFile, fiRecurse))
					nNumInclude += DoSearch(fiRecurse, hti, mapVisited);
			}
		}
